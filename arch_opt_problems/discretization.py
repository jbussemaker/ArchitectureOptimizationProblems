...
class MixedIntBaseProblem(SparsenessMixin, Problem):
    ...
    def so_run(self, pop_size=100, n_eval_max=5000, show=True):
        ...
        results = minimize(self, algorithm, termination=('n_eval', n_eval_max), save_history=True)
        f_best = [algo.opt[0].F[0] for algo in results.history]
        x = np.arange(0, len(f_best))

        plt.figure()
        plt.plot(x, f_best, '-k')
        plt.xlabel('Iterations'), plt.ylabel('Best objective')
